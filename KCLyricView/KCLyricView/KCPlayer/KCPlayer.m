...
static NSString *const AVPlayerRateKey = @"rate";
...
static NSString *const AVPlayerItemPlaybackBufferEmptyKey = @"playbackBufferEmpty";
static NSString *const AVPlayerItemPlaybackLikelyToKeepUpKey = @"playbackLikelyToKeepUp";
    ...
    if ([self.player canInsertItem:item.item afterItem:self.player.currentItem]) {
        ...
        if (currentItem) {
            ...
            if (currentItem.status == AVPlayerStatusUnknown) {
                ...
                if ([self.player canInsertItem:currentItem afterItem:nil]) {
...
- (void)addPlayerObserver
{
    ...
    [self.player addObserver:self forKeyPath:AVPlayerRateKey options:NSKeyValueObservingOptionNew context:nil];
...
- (void)removePlayerObserver
{
    ...
    [self.player removeObserver:self forKeyPath:AVPlayerRateKey];
...
- (void)addObserverWithAVPlayerItem:(AVPlayerItem *)item
{
    ...
    [item addObserver:self forKeyPath:AVPlayerItemPlaybackBufferEmptyKey options:NSKeyValueObservingOptionNew context:nil];
    [item addObserver:self forKeyPath:AVPlayerItemPlaybackLikelyToKeepUpKey options:NSKeyValueObservingOptionNew context:nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playerItemPlaybackStalledNotification:) name:AVPlayerItemPlaybackStalledNotification object:item];
...
- (void)removeObserverWithAVPlayerItem:(AVPlayerItem *)item
{
    ...
    [item removeObserver:self forKeyPath:AVPlayerItemPlaybackLikelyToKeepUpKey];
    [item removeObserver:self forKeyPath:AVPlayerItemPlaybackBufferEmptyKey];
...
//static KCPlayerStatus currentStatus;
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context
{
    ...
    }else if ([keyPath isEqualToString:AVPlayerItemPlaybackBufferEmptyKey]) {
...
//        !self.playerStatusDidChangedBlock? : self.playerStatusDidChangedBlock(self.status);
    ...
    }else if ([keyPath isEqualToString:AVPlayerItemPlaybackLikelyToKeepUpKey]) {
        
//        if (self.status == KCPlayerStatusPause) {
//
        //        }
//        if (self.currentItem.item != object) {
//            return;
        //        }
...
- (void)playerItemPlaybackStalledNotification:(NSNotification *)note
{
    NSLog(@"playerItemPlaybackStalledNotification");
}
